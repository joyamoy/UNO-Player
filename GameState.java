    ...
    GameState(Game game) {
       ...
       //如果方向为FORWARDS
        if (game.direction == Game.Direction.FORWARDS) {
            for (int i=0; i<game.h.length; i++) {//i小于玩家的个数
            	//初始化 numCardsInHandsOfUpcomingPlayers
                numCardsInHandsOfUpcomingPlayers[i] = game.h[(game.currPlayer + i + 1) % game.scoreboard.getNumPlayers()].size();
