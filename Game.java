...
public class Game {
     ...
     * GameState classes */
    Deck deck;//一副牌
    Hand h[];//数组，记录每个玩家手中的牌
    Card upCard;//桌子中间的那张牌
    Direction direction;//方向 FORWARDS, BACKWARDS 枚举类型
    int currPlayer;//目前的玩家
    UnoPlayer.Color calledColor;//当upCard为wild或者wild_4的时候，calledColor为需要打出的牌的颜色
    Scoreboard scoreboard;//记分板，即每个玩家的目前分数
    UnoPlayer.Color mostRecentColorCalled[];//最近打出的最多的牌的颜色
     ...
     * UnoPlayer interface.
     * 第二个参数， 一个字符串数组，每一个就是一个完全质量的包/类 ，实现了unoPlayer的接口
     */
     ...
     * 玩这场游戏从开始到结束
     * hands 应该已经被处理了在这个方法被调用之前，并且一张有效的牌已经在桌子中间
     * 当这个方法调用完后，游戏的记分板对象应该被更新 伴随着新的分数想胜利者靠近
     */
    public void play() {
        ...
        try {
            while (true) {//一直玩
                ...
                print(h[currPlayer].getPlayerName() +
                    " (" + h[currPlayer] + ")");//输出目前玩家的姓名如Fred[1]
                Card playedCard = h[currPlayer].play(this);//目前的玩家打出一张牌，this 为当前对象Game
                //然后我们进入hand.play(Game)中
                if (playedCard == null) {//如果没有出牌，则抽一张牌
                    Card drawnCard;
                    try {
                        drawnCard = deck.draw();
                    }
                    ...
                    playedCard = h[currPlayer].play(this);//在出一次
                }
                if (playedCard != null) {//如果有符合要求的牌
                    print(" plays " + playedCard + " on " + upCard + ".");
                    deck.discard(upCard);
                    upCard = playedCard;
                    if (upCard.followedByCall()) {//如果出的是wild或者wild_4，则要求下个玩家出牌的颜色，调用callcolor
                        calledColor = h[currPlayer].callColor(this);
                        mostRecentColorCalled[currPlayer] = calledColor;//然后将calledcolor存入mostRCC
                ...
                if (h[currPlayer].isEmpty()) {//然后判断玩家的牌是否已经出完了
                    int roundPoints = 0;
                    ...
                    scoreboard.addToScore(currPlayer,roundPoints);//然后存入记分板
                    println("---------------\n" + scoreboard);
                    return;
                }
